...
@Component({
    styleUrls: ['./signin.component.scss', '../../../../assets/css/stepper.scss'],
...
export class SignInComponent implements OnInit {
    ...
    async loginOtp() {
        ...
        if (valid) {
            ...
            this.service.validateAuthenticator(this.userLogin.Id, this.item.Otp).then(async (result: ResultApi) => {
                if (result && result.Type == ResultType.Success) {
                    this.formOtp = false;
                    await this.authen.login(this.userLogin, this.item.RememberMe);
                } else ToastrHelper.ErrorResult(result);
                this.processing = false;
            }, (ex: any) => {
                this.processing = false;
    ...
    forgotPassword() {
        this.dialog.WapperAsync({
            ...
            this.dialog.Alert('Thông báo', 'Hệ thống đã gửi email thiết lập lại mật khẩu vào hòm thư của bạn');
        });
    }
}
