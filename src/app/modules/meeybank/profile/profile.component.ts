...
export class ProfileComponent extends GridComponent implements OnInit {
    ...
    constructor() {
        ...
        this.properties = [
            ...
            {
                Property: 'MBRecruitmentTitle', Title: 'Vị trí ứng tuyển', Type: DataType.String,
                Format: (item: any) => {
                    let text: string = '';
                    let url = `${AppConfig.MeeyFinanceConfig.Url}/tuyen-dung/${item.SlugUrlRecruitment}`
                    if (item.TitleVn) text += '<p type="view"><a href='+url+'>' + item.TitleVn + '</a></p>';
                    return text;
                }
            },
            // {
    ...
    ngOnInit() {
        this.obj.Url = '/admin/MBankProfile/Items';
        this.render(this.obj);
    }

    quickView(item: MBankProfileEntity, type: string) {
        ...
        if (originalItem) {
            switch (type) {
                ...
                case 'view': {
                    ...
                    this.router.navigateByUrl('/admin/meeybank/recruitment/view?id=' + item.RecruitmentId, { state: { params: JSON.stringify(obj) } });
                } break;
            }
        }
    }
    ...
    updateStatus(item: MBankProfileEntity) {
        this.dialogService.WapperAsync({
            cancelText: 'Đóng',
