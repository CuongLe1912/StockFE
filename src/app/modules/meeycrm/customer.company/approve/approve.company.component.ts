...
export class MCRMCompanyApproveComponent implements OnInit {
    permissions: any[];
    @Input() params: any;
    errorMessage: string;
    ...
    public async confirm(): Promise<boolean> {
        let id = this.params && this.params['id'];
        if (id) {
            return await this.service.callApi('mlcompany', 'approve/' + id, this.item, MethodType.Post).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result)) {
                    setTimeout(() => {
    ...
    public async reject(): Promise<boolean> {
        ...
        if (id && await validation(this.item, ['Notes'])) {
            return await this.service.callApi('mlcompany', 'reject/' + id, this.item, MethodType.Post).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result)) {
                    setTimeout(() => {
