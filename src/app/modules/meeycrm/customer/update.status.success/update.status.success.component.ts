...
@Component({
    templateUrl: './update.status.success.component.html',
    styleUrls: ['./update.status.success.component.scss'],
})
export class UpdateStatusSuccessComponent implements OnInit {
    status: string;
    errorMessage: string;
    @Input() params: any;
    ...
    async confirm() {
        ...
        if (valid && this.item) {
            return await this.service.updateSuccess(this.item).then((result: ResultApi) => {
    ...
    private async loadItem() {
        ...
        this.item.Status = MCRMCustomerStatusType.Success;
        this.item.Ids = [this.entity.Id];
    }
    async toggleDisableButton() {
        this.disabled = !(await validation(this.item, ['Note'], true));
    }
}