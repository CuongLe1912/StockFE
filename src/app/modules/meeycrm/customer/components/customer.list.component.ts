...
@Component({
    selector: 'mcrm-customer-list',
    templateUrl: '../../../../_core/components/grid/grid.component.html',
})
export class MCRMCustomerListComponent extends GridComponent implements OnInit {
    obj: GridData = {
        Imports: [],
        Exports: [],
        Filters: [],
    ...
    @Input() customers: MCRMCustomerEntity[];
    ...
    constructor() {
        ...
        this.properties = [
            ...
            { Property: 'MeeyId', Title: 'MeeyId', Type: DataType.String, ColumnWidth: 80, DisableOrder: true }, 
            {
                Property: 'Code', Title: 'Mã khách hàng', Type: DataType.String, ColumnWidth: 120, DisableOrder: true,
                Format: ((item: any) => {
                    let text = '';
                    if (item.Code) {
                        if (this.statusRequest == MCRMCustomerRequestStatusType.Accept ||
                            this.statusRequest == MCRMCustomerRequestStatusType.Reject) {
                            if (item.Id == this.rootId) {
                                text += '<p><a routerLink="quickView" type="user">' + item.Code + '</a></p>';
                            } else {
                                text += '<p>' + item.Code + '</p>';
                            }
                        }
                        else text += '<p><a routerLink="quickView" type="user">' + item.Code + '</a></p>';
                    };
            ...
            let action = {
                ...
                click: (item: any) => {
                    this.dialogService.Confirm('Bạn có chắc chắn muốn xóa khách hàng này khỏi danh sách điều chuyển?', () => {
                        ...
                        this.items.forEach((item: MCRMCustomerEntity, index: number) => {
                            item.Index = index + 1;
                        });
    ...
    reloadItems(customers: BaseEntity[]) {
        if (customers && customers.length > 0) {
            customers.forEach((item: MCRMCustomerEntity, index: number) => {
                item.Index = index + 1;
            });
        }
