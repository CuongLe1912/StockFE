...
import { MAFApproveInvoiceComponent } from './approve.invoice/approve.invoice.component';
...
export class MAFCommissionBusinessesComponent extends GridComponent implements OnInit {
  obj: GridData = {
    ...
    Actions: [
      {
        ...
        systemName: ActionType.MAFVerifyInvoice,
        hidden: (item) => {
          if (item?.Invoice) {
            return !((item.Status == MAFStatusCommissionType.Collected || item.Status == MAFStatusCommissionType.Approved) && item?.Invoice?.Status == MAFInvoiceStatus.NotYet)
          } else return !(item.Status == MAFStatusCommissionType.Collected || item.Status == MAFStatusCommissionType.Approved)
  ...
  constructor(public userService: MLUserService) {
    ...
    this.breadcrumbs.push({ Name: 'Danh sách hoa hồng Doanh nghiệp' })
  }
  ...
  async ngOnInit() {
    ...
    this.obj.Url = '/admin/mafaffiliatesynthetic/CommissionItems?type=' + MAFContractType.Businesses;
  ...
  async loadComplete() {
    ...
    let action = "CommissionCollected?type=" + MAFContractType.Businesses;
  ...
  approve(item) {
    this.dialogService.WapperAsync({
      ...
      title: 'Duyệt hóa đơn',
      object: MAFApproveInvoiceComponent,
  ...
  export() {
    this.loading = true
    let urlExport = '/admin/mafaffiliatesynthetic/ExportCommissionItems?type=' + MAFContractType.Businesses;
    let objData: TableData = this.itemData;
    objData.Export = {
      Type: ExportType.Excel,
    }
    objData.Name = "Danh sách hoa hồng Doanh nghiệp"
    let fileName = "Danh sách hoa hồng Doanh nghiệp_" + new Date().getTime();
