...
import { MPTransactionApproveComponent } from '../../../meeypay/transactions/components/transaction.approve/transaction.approve.component';
import { MPTransactionAddNoteComponent } from '../../../meeypay/transactions/components/transaction.add.note/transaction.add.note.component';
import { MPTransactionWithdrawalCensorshipComponent } from '../../../meeypay/transactions/components/transaction.withdrawal/transaction.withdrawal.component';
...
export class MOTransactionRewardDetailComponent extends EditComponent implements OnInit {
  @Input() params: any;
  id: number;
  approve: boolean;
  item: any;
...
//   eventService: MPTransactionEventService;
  ...
  constructor(public service: MOTransactionRewardService) {
    super();
  ...
  private async renderActions() {
    let actions: ActionData[] = [
      ActionData.back(() => {
        this.router.navigate(['/admin/transactionreward']);
      }),
    ];
  ...
  private async loadItem() {
    if (this.id) {
      ...
      await this.service.item("transactionreward", this.id).then((result: ResultApi) => {
        if (ResultApi.IsSuccess(result)) {
          this.item = result.Object
          let option: OptionItem = ConstantHelper.MO_TRANSACTION_REWARD_STATUS_PAYMENTMETHOD_TYPES.find((c) => c.value == this.item.Transaction.Type);
          this.typeTransaction = option.label;
          let option1: OptionItem = ConstantHelper.MO_TRANSACTION_REWARD_STATUS_TYPES.find((c) => c.value == this.item.Transaction.Status);
          this.statusTransaction = '<p class="' + (option1 && option1.color) + '">' + (option1 && option1.label) + "</p>";
  ...
  viewSeri(seri: string) {
    let obj: NavigationStateData = {
      prevUrl: '/admin/mlcouponhmc',
    };
    let url = AppConfig.getProtocol() + '//' + AppConfig.getDomain() + '/admin/mlcouponhmc';
    this.setUrlState(obj, 'mlcouponhmc');
    window.open(url, "_blank");
  }
