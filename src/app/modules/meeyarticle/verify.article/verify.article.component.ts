...
export class MLVerifyArticleComponent implements OnInit {
    ids: number[] = [];
    @Input() params: any;
    ...
    ngOnInit() {
        ...
        this.ids = this.items && this.items.map(c => { return c.Id });
    }
    ...
    async confirm() {
        if (this.ids && this.ids.length > 0) {
            return await this.service.verifyArticle(this.ids).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result)) {
                    if (result.Object == true) {
                        let message = 'Xác minh ' + this.ids.length + ' tin thành công';
